...
export function IngestForm() {
  ...
  async function submit() {
    ...
    const res = await fetch("/api/ingest", {
      ...
      body: JSON.stringify({ platform: "youtube", query: query || undefined, handles: handleList.length ? handleList : undefined }),
    });
    ...
    <Card>
      <CardContent className="space-y-4 p-4">
        ...
        <Button onClick={submit} disabled={loading || (!query && !handles.trim())} className="w-full">
          {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Download className="h-4 w-4" />} Ingest from YouTube
        </Button>
